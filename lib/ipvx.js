  ...
  this.toDec(address, function(err, dAddress) {
    for(var i = (dAddress.length - 1); i >= 0; i--) {
      if (dAddress[i] === IPV6_MAX_BLOCK_SIZE && dAddress[i - 1] < IPV6_MAX_BLOCK_SIZE) { // Filled
        ...
        break;
      } else if (dAddress[i] < IPV6_MAX_BLOCK_SIZE) {
        dAddress[i]++;
        break;
      }
    }
...
IPv6.expandAddress = function(address, callback) {
  ...
  function checkAddress(address, callback) {
    if(IPv6.validAddress(address)) {
      let addressArray, addressLen;

      if (typeof address !== 'object') {
        addressArray = address.split(':');
        addressLen = addressArray.length;

        return callback(null, addressArray, addressLen);
      } else {
        addressLen = address.length;

        return callback(null, address, addressLen);
      } 
    } else {
  ...
  checkAddress(address, function(err, addressArray, addressLen) {
    if (err) {
      return callback(err);
    }
    ...
    try {
      ...
      for (let block = 0; addressLen > block; block++) {
...
/*
 * Check if string or array IPv6 is in range
 * @param address   IPv6 address as string or object
 * @param range     Array with start and end for range, can contain string or array
 * @param callback  callback(error, result)
 * @returns         function(error, result)
 */
IPv6.inRange = function (address, range, callback) {
  let _this = this;

  function checkRange(address, range) {

    let decRange = [];
    _this.toDec(range[0], function(err, newRange) {
      if (err) { return callback(err); }
      decRange[0] = newRange;
      _this.toDec(range[1], function(err, newRange) {
        if (err) { return callback(err); }
        decRange[1] = newRange;
        _this.toDec(address, function(err, decAddress) {
          if (err) { return callback(err); }

          for(let i = 0; i < decAddress.length; i++) {
            if (decAddress[i] === decRange[0][i] && decAddress[i] === decRange[1][i]) {
              // Same prefix, continue
            } else if (decAddress[i] < decRange[0][i] || decAddress[i] > decRange[1][i]) {
              return callback(null, false); // Not in range, less than start or larger than end
            } else if ((decAddress.length -1) === i) {
              return callback(null, true); // Last run, still in range, return true
            }
          }
        });
      });
  ...
  if (typeof range !== 'object') {
    ...
    for(let i = 0; i < 2; i++) {
      if (typeof range[i] === 'string') {
        this.expandAddress(range[i], function(err, arrRange) {
  ...
  if (typeof address === 'string') {
    this.expandAddress(address, function(err, arrAddress) {
      if (err) {
        return callback(err);
      }

      return checkRange(arrAddress, range);
    });
  } else if (typeof address === 'object') {
    address = address.slice();
  
    checkRange(address, range);
  } else {
...
/*
 * Check if string or array IPv4 is in range
 * @param address   IPv4 address as string or object
 * @param range     Array with start and end for range, can contain string or array
 * @param callback  callback(error, result)
 * @returns         function(error, result)
 */
IPv4.inRange = function (address, range, callback) {
  let _this = this;

  function checkRange(address, range) {

    for(let i = 0; i < address.length; i++) {
      if (address[i] === range[0][i] && address[i] === range[1][i]) {
        // Same prefix, continue
      } else if (address[i] < range[0][i] || address[i] > range[1][i]) {
        return callback(null, false); // Not in range, less than start or larger than end
      } else if ((address.length -1) === i) {
        return callback(null, true); // Last run, still in range, return true
      }
    }
  ...
  if (typeof range !== 'object') {
    ...
    for(let i = 0; i < 2; i++) {
      if (typeof range[i] === 'string') {
        this.expandAddress(range[i], function(arrRange) {
          range[i] = arrRange;
        });
      }
    }
  ...
  if (typeof address === 'string') {
    this.expandAddress(address, function(arrAddress) {
      return checkRange(arrAddress, range);
    });
  } else if (typeof address === 'object') {
    checkRange(address, range);
