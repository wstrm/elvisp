...
var IPTD = function IPTD (config) {
  ...
  fs.exists(_this.config.db.path, function(result) {
    ...
    if (!result) { // DB Path does not exist, looks like a dry run
      ...
      _this.tmp.lastIP = _this.config.iptunnel.range[0];
      _this.db().write('tmp', IPTD.tmp, function(err) {
        if (err) {
          throw new Error(err);
        }
    ...
    } else { // DB Path does exist, read tmp file
      _this.db().read('tmp', function(err, data) {
        if (err) {
          throw new Error(err);
        }
...
IPTD.prototype.register = function register (data, callback) {
  ...
  function addUser(address, password) {
    _this.db().write(data.pubkey, {
      ...
      // Update last IP in database and tmp
      _this.tmp.lastIP = address;
      _this.db().write('tmp', _this.tmp, function(err) {
        if (err) {
          return callback(err);
        }
  ...
  _this.db().exists(data.pubkey, function(result) {
    ...
    if (result) {
      ...
      if (_this.config.iptunnel.type === 6)   {
        ipv6.expandAddress(_this.tmp.lastIP, function(err, address) {
        ...
        ipv4.expandAddress(_this.tmp.lastIP, function(err, address) {
