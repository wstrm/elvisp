...
var IPTD = function IPTD (config) {
  ...
  // Initialize database
  let _this = this;
  fs.exists(_this.config.db, function(result) {

    if (!result) { // DB Path does not exist, looks like a dry run
      fs.mkdir(_this.config.db, DB_DIR_MODE, function(err) {
        if (err) {
          throw new Error(err);
        }

        lastIP = _this.config.iptunnel.range[0];

        _this.db().write('last_ip', { lastIP: lastIP }, function(err) {
          if (err) {
            throw new Error(err);
          }
        });
  ...
  var server = net.createServer(function(connection) { //'connection' listener
    ...
    connection.on('data', function(data) {
      ...
      if (data.password === _this.config.password) {
        ...
        _this.register(data, function(err, address) {
          if (err) {
            log.warn(err);
          }
          ...
          connection.write(JSON.stringify({
            ...
            data: {
              ...
              pubkey: err ? undefined : _this.config.cjdns.pubkey
            },
            status: err ? 0 : 1
          }) + '\r\n');
...
IPTD.prototype.register = function register (data, callback) {
  ...
  function saveUser(address, callback) {
  ...
  _this.db().exists(data.pubkey, function(result) {
    ...
    if (!result) {
      ...
      if (_this.config.iptunnel.type === 6)   {
        ...
        ipv6.addBit(lastIP, function(err, address) {
          if (err) {
            throw new Error(err);
          }
          ...
          ipv6.inRange(address, range, function(err, result) {
            ...
            ipv6.toString(address, function(err, stringAddress) {
              ...
              var registration = _this.cjdns.ipTunnel.allowConnection({ 
                ip6Address: stringAddress,
                ip6Prefix: _this.config.iptunnel.prefix,
                publicKeyOfAuthorizedNode: data.pubkey
              });

              _this.cjdns.once(registration, function(res) {
                if (res.errors.length > 0) {
                  return callback(res.errors);
                } else {
                
                  saveUser(address, function() {
  ...
  return {
    ...
    read: function read(file, callback) {
      ...
      if (file === null) {
        ...
        fs.readdir(_this.config.db, function(err, files) {
          if (err) {
            return callback(err);
          }

          for(let file = 0; file < files.length; file++) {
            fs.readFile(_this.config.db + '/' + files[file], function(err, buffer) {
              if (err) {
                return callback(err);
              }
        ...
        fs.readFile(_this.config.db + '/' + file, function(err, buffer) {
          if (err) {
            return callback(err);
          }
    ...
    exists: function exists(file, callback) {
      fs.exists(_this.config.db + '/' + file, function(result) {
    ...
    write: function write(file, data, callback) {
      ...
      fs.writeFile(_this.config.db + '/' + file, data, function(err) {
        if (err) {
          return callback(err);
        }
