...
var IPTD = function IPTD (config) {
  ...
  // Initialize database
  let iptd = this;
  if (!fs.existsSync(iptd.config.db)) {
    fs.mkdirSync(iptd.config.db, DB_DIR_MODE);
    
    this.lastIP = iptd.config.iptunnel.range[0];
    iptd.db.write('last_ip', { lastIP: this.lastIP }, throwErr);
  
  } else {
    iptd.db.read('last_ip', function(err, data) {
      throwErr(err);
...
IPTD.prototype.listen = function listen (optListen) {
  ...
  var server = net.createServer(function(connection) { //'connection' listener
    ...
    connection.on('data', function(data) {
      ...
      if (data.password === iptd.config.password) {
        log.info('Client authenticated');

        iptd.register(data, function(err, address) {
          if (err) {
            log.warn(err);
          }
          ...
          connection.write(JSON.stringify({
            ...
            data: {
              ...
              pubkey: err ? undefined : iptd.config.cjdns.pubkey
            },
            status: err ? 0 : 1
          }) + '\r\n');
  ...
  server.listen(iptd.config.listen, function() { //'listening' listener
...
IPTD.prototype.addUser = function addUser(address, pubkey, callback) {
  ...
  ipv6.toString(address, function(err, stringAddress) {
    ...
    var registration = cjdns.ipTunnel.allowConnection({ 
      ...
      ip6Prefix: iptd.config.iptunnel.prefix,
...
IPTD.prototype.register = function register (data, callback) {
  let iptd = this,
      range = iptd.config.iptunnel.range;
  ...
  iptd.db.exists(data.pubkey, function(result) {

    if (!result) {
      log.info('Unique pubkey, will register ' + data.pubkey);
      log.info('Last IP registered ' + iptd.lastIP);

      if (iptd.config.iptunnel.type === 6)   {
        log.info('Using IP type 6');

        ipv6.addBit(iptd.getLastIP(), function(err, address) {
          throwErr(err);
          ...
          ipv6.inRange(address, range, function(err, result) {
            ...
            iptd.writeUser(address, data.pubkey, data.misc);
            iptd.addUser(address, data.pubkey, callback);
          });
        });
...
IPTD.prototype.reload = function init(callback) {
  ...
  iptd.db.read(null, function(err, data) {
    if (err) {
    ...
    if (data && data.ip && data.pubkey) {
      ...
      iptd.addUser(data.ip, data.pubkey);
    }
  });
};
